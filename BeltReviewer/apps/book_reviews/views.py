    ...
    if not 'logged' in request.session:
        ...
        if request.session['logged'] == True:
            context = {
                ...
                "recent_reviews": Review.objects.all().order_by('id').reverse()[:3],
...
def book_profile(request, book_id):
    if request.session['logged'] == True:
        ...
        context = {
            "book": Book.objects.get(id = book_id),
            "reviews": Review.objects.filter(of_book = book)
        }
        return render(request, 'book_reviews/book_profile.html', context)
    else:
        return redirect('/')
...
def user_profile(request, user_id):
    if request.session['logged'] == True:
        ...
        return render(request, 'book_reviews/user_profile.html', context)
    else:
        return redirect('/')
...
def add(request):
    ...
    if book_errors or review_errors:
        ...
        if request.POST['author_list'] != "None":
            new_book = Book.objects.create(title = request.POST['title'], author = Book.objects.get(id = request.POST['author_list']).author, uploader = this_uploader)
            Review.objects.create(content = request.POST['content'], rating = int(request.POST['rating']), reviewer = this_uploader, of_book = new_book)
...
def add_review(request, book_id):
    ...
    if review_errors:
        ...
        Review.objects.create(content = request.POST['content'], rating = int(request.POST['rating']), reviewer = this_reviewer, of_book = this_book)
...
def logout(request):
    ...
    return render(request, 'book_reviews/logout.html', context)
